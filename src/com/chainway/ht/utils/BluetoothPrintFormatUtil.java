...
public class BluetoothPrintFormatUtil {
	 ...
	 * 
	 * @param menuMsgMap
	 *            Key为餐次 Value为 菜谱字符串数组 格式为：豆沙包$数量$单价
	 * @return
	 */
	public static String printMenuMSG(
		...
		int middleNumTxtLength = (LINE_BYTE_SIZE - leftPrefixLength
				- rightPrefixLength - getBytesLength(numTxt)) / 2;
		...
		for (Entry<String, LinkedList<String>> entry : menuMsgMap.entrySet()) {
			...
			for (String menu : entry.getValue()) {
				if (menu.contains(SEPARATOR)) {
					...
					if (menus != null && menus.length != 0) {
						sb.append(menus[0]);
						for (int i = 0; i < (leftPrefixLength
	...
	private static int getBytesLength(String msg) {
		try {
			return msg.getBytes("GB2312").length;
